// js/importantTasksPage.js
// 「重要任務」頁面模組：跨所有主題，僅顯示 isImportant=1 的任務

var importantTasksPage = (function () {
    'use strict';
    ...
    function isOwner(task) {
        ...
        if (!empId || !task.ownerId) return false;
        return task.ownerId.split(',').map(function (s) { return s.trim(); }).indexOf(empId.trim()) >= 0;
    }
    ...
    function loadPrerequisitesForVisibleTasks(list) {
        var ns = 'importantTasksPage';
        list.forEach(function (task) {
            if (prereqCache[task.id] !== undefined) return;
            prereqCache[task.id] = null;
            loadPrerequisites(task, function (result) {
                ...
                if (result.length === 0) {
                    ...
                    if (addZoneEl) {
                        if (owner) {
                            addZoneEl.innerHTML = '<button class="prereq-add-btn" style="margin-top:4px;" onclick="' + ns + '.openPrereqModal(' + task.id + ',null,null); event.stopPropagation();">＋ 新增前置任務</button>';
    ...
    function openPrereqModal(taskId, memoId, currentMemo) {
        ...
        if (!task) return;
        if (!isOwner(task)) { alert('只有負責人才可以編輯前置任務'); return; }
    ...
    function renderTaskList(list, containerId) {
        ...
        if (list.length === 0) {
            var msgs  = { allTaskList: '目前沒有重要任務', notStartedTaskList: '尚無未開始任務', inProgressTaskList: '尚無進行中任務', overdueTaskList: '尚無逾期任務', completedTaskList: '尚無已完成任務' };
        ...
        list.forEach(function (task, index) {
            ...
            var actualStartHTML;
            if (task.actualStart) {
                actualStartHTML = owner
                    ? '<div class="task-meta-value datetime-display clickable" ' +
                      'onclick="' + ns + '.openStartDateModal(' + task.id + '); event.stopPropagation();">' +
                      formatDateTimeDisplay(task.actualStart) + '</div>'
                    : '<div class="task-meta-value datetime-display">' + formatDateTimeDisplay(task.actualStart) + '</div>';
            } else {
                actualStartHTML = owner
            ...
            var prereqWrapperStyle = '';
            var prereqSectionContent = '';
            var prereqAddZoneStyle = 'display:none;';
            var prereqAddZoneContent = '';

            if (cached === undefined || cached === null) {
                prereqSectionContent = '<div class="task-prereq-loading">⏳ 載入中...</div>';
            } else if (cached.length === 0) {
                prereqWrapperStyle = 'display:none;';
                if (owner) {
                    prereqAddZoneStyle   = '';
                    prereqAddZoneContent = '<button class="prereq-add-btn" style="margin-top:4px;" onclick="' + ns + '.openPrereqModal(' + task.id + ',null,null); event.stopPropagation();">＋ 新增前置任務</button>';
                }
            } else {
                prereqSectionContent = '<ul class="task-prereq-list">';
                cached.forEach(function (p) {
                    prereqSectionContent += '<li class="task-prereq-item">' + prereqCodeSpanHtml(p.memo);
                    if (owner) {
                        prereqSectionContent +=
                            '<button class="prereq-btn prereq-edit-btn" style="background:#2563eb;color:#fff;border:none;border-radius:4px;padding:3px 10px;font-size:12px;cursor:pointer;margin-left:6px;" ' +
                            'onclick="' + ns + '.openPrereqModal(' + task.id + ',' + p.memoId + ',\'' + escapeAttr(p.memo) + '\'); event.stopPropagation();" title="編輯">✏️ 編輯</button>' +
                            '<button class="prereq-btn prereq-del-btn" style="background:#dc2626;color:#fff;border:none;border-radius:4px;padding:3px 10px;font-size:12px;cursor:pointer;margin-left:4px;" ' +
                            'onclick="' + ns + '.deletePrerequisite(' + p.memoId + ',' + task.id + '); event.stopPropagation();" title="刪除">🗑️ 刪除</button>';
                    }
                    prereqSectionContent += '</li>';
                    if (PREREQ_CODE_REGEX.test(p.memo)) resolvePrereqLink(p.memo);
                });
                prereqSectionContent += '</ul>';
                if (owner) {
                    prereqSectionContent += '<button class="prereq-add-btn" onclick="' + ns + '.openPrereqModal(' + task.id + ',null,null); event.stopPropagation();">＋ 新增前置任務</button>';
                }
            }
            ...
            item.innerHTML =
                ...
                '<div class="task-prereq-wrapper" style="' + prereqWrapperStyle + '">' +
                '<div class="task-prereq-label">🔗 等候前置任務</div>' +
                '<div class="task-prereq-section" data-task-id="' + task.id + '">' + prereqSectionContent + '</div>' +
                '</div>' +
                '<div class="task-prereq-add-zone" data-task-id="' + task.id + '" style="' + prereqAddZoneStyle + '">' + prereqAddZoneContent + '</div>' +
                '</div></div>' +
                noteHTML;
    ...
    function toggleTask(taskId) {
        ...
        if (!task.completed) {
            ...
            document.getElementById('modalActualDate').value =
                ...
                String(now.getHours()).padStart(2,'0') + ':' + String(now.getMinutes()).padStart(2,'00');
    ...
    // ─── 初始化 ───────────────────────────────────────────────────────────
    function init() {
        // 根據 URL groupPage 參數顯示頁面副標題
        var params = getUrlParams();
        var groupPageFilter = params.groupPage || '';
        var subtitleEl = document.getElementById('importantTasksSubtitle');
        if (subtitleEl) {
            if (groupPageFilter && CATEGORY_LABELS[groupPageFilter]) {
                subtitleEl.textContent = CATEGORY_LABELS[groupPageFilter].name + '・重要任務清單';
            } else {
                subtitleEl.textContent = '全主題・重要任務清單';
            }
        }
...
document.addEventListener('DOMContentLoaded', function () {
    importantTasksPage.init();
});

window.confirmComplete   = importantTasksPage.confirmComplete;
window.cancelComplete    = importantTasksPage.cancelComplete;
window.confirmUncomplete = importantTasksPage.confirmUncomplete;
window.cancelUncomplete  = importantTasksPage.cancelUncomplete;
