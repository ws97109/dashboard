...
var p2Page = (function() {
    ...
    function loadNotesFromStorage() {
        var savedNotes = localStorage.getItem('taskNotes2');
    ...
    function saveNotesToStorage() {
        ...
        localStorage.setItem('taskNotes2', JSON.stringify(notes));
    }
    ...
    function loadTasksData() {
        ...
        DataLoader.loadProjectData(function(err, data) {
            ...
            var category = data.categories.find(function(cat) { return cat.id === 2; });
    ...
    function renderTaskList(tasks, containerId) {
        ...
        tasks.forEach(function(task, index) {
            ...
            var actualStartHTML = '';
            if (task.actualStart) {
                actualStartHTML = '<div class="task-meta-value datetime-display clickable" onclick="p2Page.openStartDateModal(' + task.id + '); event.stopPropagation();">' + 
                    formatDateTimeDisplay(task.actualStart) + 
                    '</div>';
            } else {
                actualStartHTML = '<div class="task-meta-value">' +
                    '<button class="set-date-btn" onclick="p2Page.openStartDateModal(' + task.id + '); event.stopPropagation();">設定開始日期</button>' +
                    '</div>';
            }
            ...
            taskItem.innerHTML = 
                ...
                '<button class="' + noteButtonClass + '" onclick="p2Page.openNoteModal(' + task.id + '); event.stopPropagation();">' +
                noteButtonText +
                '</button>' +
                '<button class="task-update-btn" onclick="p2Page.toggleTask(' + task.id + '); event.stopPropagation();">' +
                '更新進度' +
                '</button>' +
                '</div>' +
...
window.confirmComplete = p2Page.confirmComplete;
window.cancelComplete = p2Page.cancelComplete;
window.confirmUncomplete = p2Page.confirmUncomplete;
window.cancelUncomplete = p2Page.cancelUncomplete;