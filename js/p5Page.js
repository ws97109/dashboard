...
var p5Page = (function() {
    ...
    function loadNotesFromStorage() {
        var savedNotes = localStorage.getItem('taskNotes5');
    ...
    function saveNotesToStorage() {
        ...
        localStorage.setItem('taskNotes5', JSON.stringify(notes));
    }
    ...
    function renderTaskList(tasks, containerId) {
        ...
        tasks.forEach(function(task, index) {
            ...
            var actualStartHTML = '';
            if (task.actualStart) {
                actualStartHTML = '<div class="task-meta-value datetime-display clickable" onclick="p5Page.openStartDateModal(' + task.id + '); event.stopPropagation();">' + 
                    formatDateTimeDisplay(task.actualStart) + 
                    '</div>';
            } else {
                actualStartHTML = '<div class="task-meta-value">' +
                    '<button class="set-date-btn" onclick="p5Page.openStartDateModal(' + task.id + '); event.stopPropagation();">設定開始日期</button>' +
                    '</div>';
            }
            ...
            taskItem.innerHTML = 
                ...
                '<button class="' + noteButtonClass + '" onclick="p5Page.openNoteModal(' + task.id + '); event.stopPropagation();">' +
                noteButtonText +
                '</button>' +
                '<button class="task-update-btn" onclick="p5Page.toggleTask(' + task.id + '); event.stopPropagation();">' +
                '更新進度' +
                '</button>' +
                '</div>' +
...
window.confirmComplete = p5Page.confirmComplete;
window.cancelComplete = p5Page.cancelComplete;
window.confirmUncomplete = p5Page.confirmUncomplete;
window.cancelUncomplete = p5Page.cancelUncomplete;