// js/allTasksPage.js
// 「全部任務」頁面模組：跨所有主題，顯示全部任務，可依 system_code 篩選
...
var allTasksPage = (function () {
    ...
    var searchQuery      = '';
    var systemCodeFilter = '';   // ← 系統代碼精確篩選（datalist 輸入）
    ...
    function renderTaskList(list, containerId) {
        ...
        if (list.length === 0) {
            var msgs  = { allTaskList: '目前沒有任務資料', notStartedTaskList: '尚無未開始任務', inProgressTaskList: '尚無進行中任務', overdueTaskList: '尚無逾期任務', completedTaskList: '尚無已完成任務' };
        ...
        list.forEach(function (task, index) {
            ...
            if (task.actualStart) {
                if (isOwner) {
                    ...
                    actualStartHTML = '<div class="task-meta-value datetime-display">' +
                        formatDateTimeDisplay(task.actualStart) + '</div>';
                }
            } else {
                if (isOwner) {
    ...
    function loadMemosForVisibleTasks(list) {
        list.forEach(function (task) {
            ...
            loadMemos(task, function () {
                ...
                btn.setAttribute('onclick', 'allTasksPage.openNoteSection(' + task.id + '); event.stopPropagation();');
            });
        });
    }
    ...
    function openStartDateModal(taskId) {
        ...
        if (task.actualStart) {
            ...
            if (!isNaN(dt.getTime())) {
                ...
                hourSelect.value = String(dt.getHours()).padStart(2, '00');
                minSelect.value  = String(dt.getMinutes()).padStart(2, '00');
            }
        } else {
            var now = new Date();
            dateInput.value  = now.getFullYear() + '-' + String(now.getMonth()+1).padStart(2,'0') + '-' + String(now.getDate()).padStart(2,'0');
            hourSelect.value = String(now.getHours()).padStart(2, '00');
            minSelect.value  = String(now.getMinutes()).padStart(2, '00');
        }
        syncStartDateHidden();
    ...
    function updateStats() {
        ...
        var inProgress = tasks.filter(function (t) {
            ...
            return new Date() >= new Date(t.actualStart);
        }).length;
        ...
        container.innerHTML = '';
        [
            { value: pct + '%',              label: '整體完成度' },
            { value: completed + '/' + total, label: '已完成任務' },
            { value: inProgress + '',         label: '進行中任務' },
...
window.confirmComplete   = allTasksPage.confirmComplete;
window.cancelComplete    = allTasksPage.cancelComplete;
window.confirmUncomplete = allTasksPage.confirmUncomplete;
window.cancelUncomplete  = allTasksPage.cancelUncomplete;
