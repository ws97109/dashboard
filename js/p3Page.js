...
var p3Page = (function() {
    ...
    function loadNotesFromStorage() {
        var savedNotes = localStorage.getItem('taskNotes3');
    ...
    function saveNotesToStorage() {
        ...
        localStorage.setItem('taskNotes3', JSON.stringify(notes));
    }
    ...
    function loadTasksData() {
        ...
        DataLoader.loadProjectData(function(err, data) {
            ...
            var category = data.categories.find(function(cat) { return cat.id === 3; });
                        ...
                        if (group.tasks) {
                            group.tasks.forEach(function(task) {
                                ...
                                var matchTaskType = !selectedTaskType || 
                                    (selectedTaskType === '資料轉置') ||
                                    (selectedTaskType === '系統切轉');
    ...
    function renderTaskList(tasks, containerId) {
        ...
        tasks.forEach(function(task, index) {
            ...
            var actualStartHTML = '';
            if (task.actualStart) {
                actualStartHTML = '<div class="task-meta-value datetime-display clickable" onclick="p3Page.openStartDateModal(' + task.id + '); event.stopPropagation();">' + 
                    formatDateTimeDisplay(task.actualStart) + 
                    '</div>';
            } else {
                actualStartHTML = '<div class="task-meta-value">' +
                    '<button class="set-date-btn" onclick="p3Page.openStartDateModal(' + task.id + '); event.stopPropagation();">設定開始日期</button>' +
                    '</div>';
            }
            ...
            taskItem.innerHTML = 
                ...
                '<button class="' + noteButtonClass + '" onclick="p3Page.openNoteModal(' + task.id + '); event.stopPropagation();">' +
                noteButtonText +
                '</button>' +
                '<button class="task-update-btn" onclick="p3Page.toggleTask(' + task.id + '); event.stopPropagation();">' +
                '更新進度' +
                '</button>' +
                '</div>' +
...
window.confirmComplete = p3Page.confirmComplete;
window.cancelComplete = p3Page.cancelComplete;
window.confirmUncomplete = p3Page.confirmUncomplete;
window.cancelUncomplete = p3Page.cancelUncomplete;