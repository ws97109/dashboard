...
var p4Page = (function() {
    ...
    function loadNotesFromStorage() {
        var savedNotes = localStorage.getItem('taskNotes4');
    ...
    function saveNotesToStorage() {
        ...
        localStorage.setItem('taskNotes4', JSON.stringify(notes));
    }

    function loadTaskDatesFromStorage() {
        var savedDates = localStorage.getItem('taskDates4');
    ...
    function saveDatesToStorage() {
        ...
        localStorage.setItem('taskDates4', JSON.stringify(dates));
    }
    ...
    function loadTasksData() {
        ...
        DataLoader.loadProjectData(function(err, data) {
            ...
            var category = data.categories.find(function(cat) { return cat.id === 4; });
    ...
    function renderTaskList(tasks, containerId) {
        ...
        tasks.forEach(function(task, index) {
            ...
            var actualStartHTML = '';
            if (task.actualStart) {
                actualStartHTML = '<div class="task-meta-value datetime-display clickable" onclick="p4Page.openStartDateModal(' + task.id + '); event.stopPropagation();">' + 
                    formatDateTimeDisplay(task.actualStart) + 
                    '</div>';
            } else {
                actualStartHTML = '<div class="task-meta-value">' +
                    '<button class="set-date-btn" onclick="p4Page.openStartDateModal(' + task.id + '); event.stopPropagation();">設定開始日期</button>' +
                    '</div>';
            }
            ...
            taskItem.innerHTML = 
                ...
                '<button class="' + noteButtonClass + '" onclick="p4Page.openNoteModal(' + task.id + '); event.stopPropagation();">' +
                noteButtonText +
                '</button>' +
                '<button class="task-update-btn" onclick="p4Page.toggleTask(' + task.id + '); event.stopPropagation();">' +
                '更新進度' +
                '</button>' +
                '</div>' +
...
window.confirmComplete = p4Page.confirmComplete;
window.cancelComplete = p4Page.cancelComplete;
window.confirmUncomplete = p4Page.confirmUncomplete;
window.cancelUncomplete = p4Page.cancelUncomplete;